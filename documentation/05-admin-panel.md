...
### Kjent, halvferdig migrering: paginering

`app/api/admin/users/route.ts` (GET) bygger et `AdminUserQueryParams`-objekt (`search`, `statusFilter`,
`page`, `pageSize`) og forventer at `agentAuthAdmin.getUsers()` returnerer en `PaginatedResponse<AdminUserListItem>`
(`{ totalItems, page, pageSize, totalPages, items }`). Men `agentAuthAdmin.getUsers()` er per nå fortsatt:
...
— den tar ingen parametre og returnerer en flat liste, ikke et paginert objekt. Dette gir faktiske
TypeScript-feil ved `npx tsc --noEmit` i dag. Modellene (`AdminUserQueryParams`, `PaginatedResponse`) er
allerede lagt til, så dette er en påbegynt, ikke fullført migrering til server-side paginering. Se
[07 – Kjente problemer](./07-known-issues-and-tech-debt.md#halvferdig-paginering-av-brukerlisten).
